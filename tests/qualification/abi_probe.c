...
static uint32_t (COGGATE_CALL *const ABI_VERSION)(void) = ag_abi_version;
static ag_byte_slice (COGGATE_CALL *const CORE_VERSION)(void) = ag_core_version;
static ag_status (COGGATE_CALL *const BUFFER_FREE)(ag_owned_buffer *) = ag_buffer_free;
static ag_status (COGGATE_CALL *const SERVICE_CREATE)(
    const ag_lifecycle_callbacks *,
    const ag_key_callbacks *,
    const ag_observer_callbacks *,
    ag_service **
) = ag_service_create;
static ag_status (COGGATE_CALL *const SERVICE_DESTROY)(ag_service *) = ag_service_destroy;
static ag_status (COGGATE_CALL *const SERVICE_ISSUE)(
    ag_service *, ag_byte_slice, ag_byte_slice, ag_attempt_limit, ag_owned_buffer *
) = ag_service_issue;
static ag_status (COGGATE_CALL *const SERVICE_VERIFY)(
