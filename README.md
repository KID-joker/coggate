...
The release crate must never depend on `agentgate-benchmark` or `agentgate-core`, and it must never enable `agentgate-core/insecure-benchmarking` or expose its oracle. The deterministic benchmark oracle is only for benchmark-crate feature tests; it must never be enabled in production services, the C ABI, or language bindings.
