...
use crate::{
    ...
    memory::{required_host_buffer_copy_count, reset_required_host_buffer_copy_count},
};

use super::{CallbackKeys, CallbackLifecycle, CallbackObserver, OpaqueAttemptToken};

#[test]
fn attempt_token_debug_is_redacted() {
    let token = OpaqueAttemptToken::new(b"secret-attempt-token".to_vec());
    let debug = format!("{token:?}");
    assert_eq!(debug, "OpaqueAttemptToken([REDACTED])");
    assert!(!debug.contains("secret-attempt-token"));
}
...
fn begin_allows_empty_token_and_releases_both_outputs_once() {
    ...
    let mut adapter = unsafe { CallbackLifecycle::new(callbacks, Arc::clone(&flag)) };
    assert!(
        adapter
...
fn active_key_utf8_failure_releases_outputs_and_marks_violation() {
    ...
    let flag = Arc::new(AtomicBool::new(false));
    let mut adapter = unsafe { CallbackKeys::new(callbacks, Arc::clone(&flag)) };
    reset_required_host_buffer_copy_count();
    assert!(adapter.active_key().is_err());
    assert_eq!(required_host_buffer_copy_count(), 1);
    assert_eq!(unsafe { &*ptr }.releases.load(Ordering::SeqCst), 2);
    assert!(flag.load(Ordering::SeqCst));
    drop(unsafe { Box::from_raw(ptr) });
}
    ...
    for (raw, expected) in [
        ...
        assert_eq!(
            fixture
                .lifecycle(Arc::new(AtomicBool::new(false)))
                .finish_attempt(
                    OpaqueAttemptToken::new(Vec::new()),
...
unsafe extern "C" fn contract_finish(data: *mut c_void, token: AgByteSlice, outcome: i32) -> i32 {
    let fixture = unsafe { &*data.cast::<ContractFixture>() };
    fixture
        .finish_calls
        .lock()
...
fn begin_and_finish_pass_exact_callback_inputs_and_outcomes() {
    ...
    for (outcome, raw) in [
        ...
        adapter
            .finish_attempt(OpaqueAttemptToken::new(b"opaque-token".to_vec()), outcome)
            .unwrap();
        assert_eq!(
...
fn active_key_rejects_invalid_ids_and_short_keys_with_exact_release() {
    ...
    for (id, key, expected_required_copies) in cases {
        let mut fixture = ContractFixture::new();
        ...
        *fixture.key.lock().unwrap() = key;
        ...
        reset_required_host_buffer_copy_count();
        assert!(fixture.keys(Arc::clone(&flag)).active_key().is_err());
        assert_eq!(required_host_buffer_copy_count(), expected_required_copies);
        assert!(flag.load(Ordering::SeqCst));
        assert_eq!(fixture.releases.load(Ordering::SeqCst), 2);
    }
}
...
fn observer_is_optional_and_forwards_safe_json_synchronously() {
    ...
    unsafe { CallbackObserver::new(None, Arc::clone(&flag)) }.observe(&event);
    assert!(!flag.load(Ordering::SeqCst));
    ...
    unsafe { CallbackObserver::new(Some(callbacks), Arc::clone(&flag)) }.observe(&event);
