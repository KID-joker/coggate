#ifndef AGENTGATE_CPP_FIXTURE_SUPPORT_HPP
#define AGENTGATE_CPP_FIXTURE_SUPPORT_HPP
...
#ifndef AGENTGATE_BINDING_FIXTURE_PATH
#error "AGENTGATE_BINDING_FIXTURE_PATH must name fixtures/bindings/v1.json"
#endif

namespace agentgate_fixture {

using Json = nlohmann::ordered_json;

inline const Json &manifest() {
  static const Json value = [] {
    std::ifstream input(AGENTGATE_BINDING_FIXTURE_PATH,
                        std::ios::in | std::ios::binary);
    if (!input) {
      throw std::runtime_error("unable to open AgentGate binding fixture");
    }
    ...
    if (!parsed.is_object() || parsed.value("fixture_version", 0) != 1 ||
      ...
      throw std::runtime_error("invalid AgentGate binding fixture");
    }
    return parsed;
  }();
  return value;
...
inline const Json &fixture_by_id(std::string_view id) {
  ...
  throw std::runtime_error("AgentGate binding fixture ID not found");
}
...
inline std::uint8_t hex_nibble(char value) {
  ...
  throw std::runtime_error("invalid hex in AgentGate binding fixture");
}
